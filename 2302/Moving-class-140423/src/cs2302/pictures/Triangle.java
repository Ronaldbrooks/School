...
/**
 * The base point is one vertex of the triangle. The other two vertices are new
 * instance fields.
 * 
 * @author bsetzer
 * 
...
public class Triangle extends Geometric {
	...
	public Triangle(PicPoint v0, PicPoint v1, PicPoint v2, Paint fill,
			Paint outline) {
	...
	public void draw(Canvas cnv) {
		...
		// trivia: vertex is from the Greek, so the plural is a Greek plural:
		// vertices
		PicPoint v0 = getBase();
		float x0 = v0.getX();
		float y0 = v0.getY();
	...
	public Picture getTouched(float x, float y) {
		...
		if (PointInTriangle(pt, getBase(), v1, v2)) {
			return this;
		} else {
			return null;
		}
	...
	static float sign(PicPoint p1, PicPoint p2, PicPoint p3) {
		return (p1.getX() - p3.getX()) * (p2.getY() - p3.getY())
				- (p2.getX() - p3.getX()) * (p1.getY() - p3.getY());

	} // getBase() = v3

	static boolean PointInTriangle(PicPoint pt, PicPoint v1, PicPoint v2,
			PicPoint v0) {
		boolean b1, b2, b3;

		b1 = sign(pt, v1, v2) < 0.0f;
		b2 = sign(pt, v2, v0) < 0.0f;
		b3 = sign(pt, v0, v1) < 0.0f;
	...
	/**
	 * Move a triangle, what changes
	 * 
	 * One vertex is the base point: change v1 change v2 change
	 */
	@Override
	public void moveRelative(float dx, float dy) {
		super.moveRelative(dx, dy); // move basepoint
		v1.moveRelative(dx, dy);
		v2.moveRelative(dx, dy);
	}

	@Override
	public void moveAbsolute(float newX, float newY) {
		super.moveAbsolute(newX, newY);
		v1.moveAbsolute(newX, newY);
		v2.moveAbsolute(newX, newY);
	}
